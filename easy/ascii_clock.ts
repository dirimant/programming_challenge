...
function numberToDigit(value: string){
    switch(value){
        case '0':
            return [" __ ", "|  |", "|__|"];
        case '1':
...
function renderClock(clockString: string){
    ...
    if(true){
        ...
        for(let i = 0; i < renderedClock.length; i++){
            for(let x = 0; x < 3; x++){
                if(x == 0) upperString = upperString.concat(renderedClock[i][x]);
                if(x == 1) middleString = middleString.concat(renderedClock[i][x]);
                if(x == 2) downString = downString.concat(renderedClock[i][x])
            }
